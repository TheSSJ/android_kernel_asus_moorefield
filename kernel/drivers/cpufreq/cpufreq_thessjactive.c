/*
 * drivers/cpufreq/cpufreq_thessjactive.c
 *
...
struct cpufreq_thessjactive_tunables {
	int usage_count;
...
static void cpufreq_thessjactive_timer_resched(
 ...
 * function.
 */
static void cpufreq_thessjactive_timer_start(
	struct cpufreq_thessjactive_tunables *tunables, int cpu)
{
	struct cpufreq_thessjactive_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
...
static unsigned int freq_to_targetload(
	struct cpufreq_thessjactive_tunables *tunables, unsigned int freq)
{
	int i;
	unsigned int ret;
	unsigned long flags;
...
static void cpufreq_thessjactive_timer(unsigned long data)
{
	u64 now;
	unsigned int delta_time;
	unsigned int cur;
	...
	if (pcpu->target_freq >= tunables->hispeed_freq &&
		...
		trace_cpufreq_thessjactive_notyet(
	...
	if (pcpu->target_freq == new_freq &&
		...
		trace_cpufreq_thessjactive_already(
			data, cpu_load, pcpu->target_freq,
			pcpu->policy->cur, new_freq);
		spin_unlock_irqrestore(&pcpu->target_freq_lock, flags);
	...
	trace_cpufreq_thessjactive_target(data, cpu_load, pcpu->target_freq,
					 pcpu->policy->cur, new_freq);
...
rearm:
	if (!timer_pending(&pcpu->cpu_timer))
		cpufreq_thessjactive_timer_resched(pcpu);
...
static void cpufreq_thessjactive_idle_start(void)
{
	struct cpufreq_thessjactive_cpuinfo *pcpu =
...
static void cpufreq_thessjactive_idle_end(void)
{
	struct cpufreq_thessjactive_cpuinfo *pcpu =
		&per_cpu(cpuinfo, smp_processor_id());
	...
	if (!timer_pending(&pcpu->cpu_timer)) {
		...
		cpufreq_thessjactive_timer(smp_processor_id());
	}

	up_read(&pcpu->enable_sem);
}

static int cpufreq_thessjactive_speedchange_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			for_each_cpu(j, pcpu->policy->cpus) {
				struct cpufreq_thessjactive_cpuinfo *pjcpu =
					&per_cpu(cpuinfo, j);
			...
			if (max_freq != pcpu->policy->cur) {
				__cpufreq_driver_target(pcpu->policy,
							max_freq,
							CPUFREQ_RELATION_H);
				trace_cpufreq_thessjactive_setspeed(cpu,
							max_freq,
							pcpu->policy->cur);
			}
...
static void cpufreq_thessjactive_boost(struct cpufreq_thessjactive_tunables *tunables)
{
	int i;
	int anyboost = 0;
...
static void cpufreq_thessjactive_touchboost(void)
{
	int i;
	int anyboost = 0;
	unsigned long flags;
...
static struct notifier_block cpufreq_notifier_block = {
	.notifier_call = cpufreq_thessjactive_notifier,
};
...
static ssize_t show_above_hispeed_delay(
	struct cpufreq_thessjactive_tunables *tunables, char *buf)
{
	int i;
	ssize_t ret = 0;
	unsigned long flags;
...
static ssize_t show_hispeed_freq(struct cpufreq_thessjactive_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->hispeed_freq);
}

static ssize_t store_hispeed_freq(struct cpufreq_thessjactive_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
...
static ssize_t show_go_hispeed_load(struct cpufreq_thessjactive_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->go_hispeed_load);
}

static ssize_t store_go_hispeed_load(struct cpufreq_thessjactive_tunables
...
static ssize_t show_touchboost_freq(struct cpufreq_thessjactive_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->touchboost_freq);
}

static ssize_t store_touchboost_freq(struct cpufreq_thessjactive_tunables
				*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_min_sample_time(struct cpufreq_thessjactive_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->min_sample_time);
}

static ssize_t store_min_sample_time(struct cpufreq_thessjactive_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_timer_rate(struct cpufreq_thessjactive_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%lu\n", tunables->timer_rate);
}

static ssize_t store_timer_rate(struct cpufreq_thessjactive_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_timer_slack(struct cpufreq_thessjactive_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%d\n", tunables->timer_slack_val);
}

static ssize_t store_timer_slack(struct cpufreq_thessjactive_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_boost(struct cpufreq_thessjactive_tunables *tunables,
			  char *buf)
{
	return sprintf(buf, "%d\n", tunables->boost_val);
}

static ssize_t store_boost(struct cpufreq_thessjactive_tunables *tunables,
			   const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	if (tunables->boost_val) {
		trace_cpufreq_thessjactive_boost("on");
		if (!tunables->boosted)
			cpufreq_thessjactive_boost(tunables);
	} else {
...
static ssize_t store_boostpulse(struct cpufreq_thessjactive_tunables *tunables,
				const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_boostpulse_duration(struct cpufreq_thessjactive_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%d\n", tunables->boostpulse_duration_val);
}

static ssize_t store_boostpulse_duration(struct cpufreq_thessjactive_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t store_touchboostpulse(struct cpufreq_thessjactive_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_touchboostpulse_duration(struct cpufreq_thessjactive_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%d\n", tunables->touchboostpulse_duration_val);
}
...
static ssize_t show_io_is_busy(struct cpufreq_thessjactive_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->io_is_busy);
}

static ssize_t store_io_is_busy(struct cpufreq_thessjactive_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static struct attribute *thessjactive_attributes_gov_sys[] = {
...
static struct attribute_group thessjactive_attr_group_gov_sys = {
	.attrs = thessjactive_attributes_gov_sys,
	.name = "thessjactive",
};

/* Per policy governor instance */
static struct attribute *thessjactive_attributes_gov_pol[] = {
...
static struct attribute_group thessjactive_attr_group_gov_pol = {
	.attrs = thessjactive_attributes_gov_pol,
	.name = "thessjactive",
};
...
static struct attribute_group *get_sysfs_attr(void)
{
	if (have_governor_per_policy())
		return &thessjactive_attr_group_gov_pol;
	else
					     ...
					     void *data)
{
	switch (val) {
	case IDLE_START:
		cpufreq_thessjactive_idle_start();
		break;
	case IDLE_END:
		cpufreq_thessjactive_idle_end();
		break;
	}
...
static struct notifier_block cpufreq_thessjactive_idle_nb = {
	.notifier_call = cpufreq_thessjactive_idle_notifier,
};

static int cpufreq_governor_thessjactive(struct cpufreq_policy *policy,
		unsigned int event)
{
	int rc;
	unsigned int j;
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			cpufreq_thessjactive_timer_start(tunables, j);
...
struct cpufreq_governor cpufreq_gov_thessjactive = {
	.name = "thessjactive",
	.governor = cpufreq_governor_thessjactive,
...
static int __init cpufreq_thessjactive_init(void)
{
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_slack_timer.function = cpufreq_thessjactive_nop_timer;
	...
	mutex_init(&gov_lock);
	speedchange_task =
		kthread_create(cpufreq_thessjactive_speedchange_task, NULL,
			       "cfthessjactive");
...
#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_thessjactive
fs_initcall(cpufreq_thessjactive_init);
#else
module_init(cpufreq_thessjactive_init);
#endif

static void __exit cpufreq_thessjactive_exit(void)
{
